<template>
    <div class="content" v-loading="isConnecting" :style="cssVars">
        ...
        <div class="scroll-to-bottom-btn" v-show="showDownBtn && danmuItems.length > 0" @click="onScrollToBottomClick">
            <SvgIcon class="btn-icon" :svg-raw="svg.downSvg" :size="'1.5em'"/>
        </div>
        <div class="pending-size" v-show="pendingSize > 0" @click="onScrollToBottomClick">{{ (pendingSize>99?'99+':pendingSize) + '新消息' }}</div>
    </div>
</template>

<script setup lang="ts">
import { computed, ref, watch, nextTick, onActivated, onDeactivated, onMounted, onUnmounted } from 'vue'
